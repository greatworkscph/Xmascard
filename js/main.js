...
$( document ).ready(function() {
  ...
  // 			self = this;
	...
	function Record(el) {
		...
		this.title = this.wrapper.querySelector('.infos-title');
		this.tracks = this.wrapper.querySelector('.infos-tracks');

		var tracklist = [];
		this.wrapper.querySelectorAll('.track').forEach(function(el) {
	...
	// Vars to be updated after Json playlist loaded.
	var views, lpGrid,lps, expanderEl, recordEl,	slideshow, turntable;
	...
	function showExpander(position, callback) {
		dynamics.css(expanderEl, { opacity: 1, left : position.x, top : position.y, backgroundColor : '#d3d5d7', scale : 0 });
	...
	//Fred -- Load json file and generate records
	...
	function generateRecords() {
		...
		$.getJSON( "js/playlists.json", function( data ) {
			...
			if( data.length > 0 ) {
				...
				for (var i = 0; i < data.length; i++) {
						...
						html += '<a class="grid__link" href="#album-' + ( i+1 ) + '">';
						html += '<div class="img-wrap img-wrap--grid">';
						html += '<svg class="lp lp--grid"><use xlink:href="#icon-lp-mini"></use></svg>';
						html += '<img class="img img--grid" src="' + pl.cover + '" /></div>';
						// html += '<h2 class="artist">' + pl.playlist + '</h2>';
						html += '<h3 class="title">' + pl.playlist + '</h3>';
						html += '</a></li>';
						...
						html = 	'<div class="single" id="album-' + i + '">';
						html += '<div class="img-wrap img-wrap--single">';
						html += '<img class="img img--single" src="' + pl.cover + '" alt="album' + i + '" />';
						html += '</div>';
						// html += '<h3 class="title title--single">' + pl.playlist + '</h3>';
						html += '<h2 class="artist artist--single">' + pl.playlist + '</h2>';
						html += '';
						...
						html += '<div class="infos" id="infos-' + i + '">';
						html += '<div class="infos-cover">';
						html += '<img src="' + pl.cover + '" />';
						html += '</div>';
						html += '<h2 class="infos-title">' + pl.playlist + '</h2>';
						html += '<div class="infos-tracks">';
						html += '<ul>';

						for (var j = 0; j < pl.tracks.length; j++) {

							var trx = pl.tracks[j];
							html += '<li id="track-' + j + '" data-url="' + trx.url + '" class="track">' + (j+1) + '. ' + trx.artist + '-' + trx.title + '</li>';
  ...
  var YTplayer;
	...
	function YTbindEvents() {
		...
		window.onYouTubeIframeAPIReady = function() {
			...
			// ex https://www.youtube.com/watch?v=oEDB6HUgaJg
			// parse video id
	...
	function initYoutube() {
		...
		// console.log('initYoutube called');
		//
    // // 3. This function creates an <iframe> (and YouTube player)
    // //    after the API code downloads.
    // function onYouTubeIframeAPIReady() {
		// 	console.log('IframeAPI called');
